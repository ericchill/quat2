...
public:
	...
	void SetInputs(Fl_Value_Input *cre, Fl_Value_Input *ci, Fl_Button *upd)
		{ _input_cre = cre; _input_ci = ci; _button_update = upd; }
	void SetCX(double cx) { int dummy = 0; _cx = cx; C2Coo(cx, 0.0, _CursorX, dummy); redraw(); }
	void SetCY(double cy) { int dummy = 0; _cy = cy; C2Coo(0.0, cy, dummy, _CursorY); redraw(); }
	void MoveUp();
	void MoveDown();
	void MoveLeft();
	void MoveRight();
	...
	void maxiter(int v)
		{ _Maxiter = v; CheckUpdate(); return; }
	int formula() const { return _Formula; }
	void formula(int v)
		{ _Formula = v; CheckUpdate(); return; }
	double bailout() const { return _Bailout; }
	void bailout(double v)
		{ _Bailout = v; _BailoutSQR = v*v; CheckUpdate(); return; }
	void p(int i, int j, double v) { _p[i][j] = v; CheckUpdate(); }
	double cj() const { return _cj; }
	void cj(double v) { _cj = v; CheckUpdate(); return; }
	double ck() const { return _ck; }
	void ck(double v) { _ck = v; CheckUpdate(); return; }
	bool updated(bool v) { _updated = v; return true; }
