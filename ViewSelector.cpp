...
void ViewSelector::calculateCoords()
{
	...
	pre = static_cast<int>(_v._s[0]*10.0);
	pi = static_cast<int>(_v._s[1]*10.0);
	pj = static_cast<int>(_v._s[2]*10.0);
	if (_x==0) _vx = pre;
	if (_x==1) _vx = pi;
	if (_x==2) _vx = pj;
	if (_y==0) _vy = pre;
	...
	if (!_v.calcbase(&base, 0, WhichEye::Monocular) && (_v._xres != 0) ) {
		/* Calculate bitmap coordinates of view rectangle */
		plane_x[0] = static_cast<int>(base._O[_x]*10.0);
		plane_x[1] = static_cast<int>((base._O[_x]+_v._LXR*base._x[_x])*10.0);
		plane_x[2] = static_cast<int>((base._O[_x]+_v._LXR*base._x[_x]
			+_v._LXR*(static_cast<double>(_v._yres)/static_cast<double>(_v._xres))*base._y[_x])*10.0);
		plane_x[3] = static_cast<int>((base._O[_x]
			+_v._LXR*(static_cast<double>(_v._yres)/static_cast<double>(_v._xres))*base._y[_x])*10.0);
		plane_y[0] = static_cast<int>(base._O[_y]*10.0);
		plane_y[1] = static_cast<int>((base._O[_y]+_v._LXR*base._x[_y])*10.0);
		plane_y[2] = static_cast<int>((base._O[_y]+_v._LXR*base._x[_y]
			+_v._LXR*(static_cast<double>(_v._yres)/static_cast<double>(_v._xres))*base._y[_y])*10.0);
		plane_y[3] = static_cast<int>((base._O[_y]
			+_v._LXR*(static_cast<double>(_v._yres)/static_cast<double>(_v._xres))*base._y[_y])*10.0);
